<?php

namespace App\Policies;

use App\Models\Crop;
use App\Models\User;
use Illuminate\Auth\Access\HandlesAuthorization;

class CropPolicy
{
    use HandlesAuthorization;

    /**
     * Determine whether the crop can view any models.
     */
    public function viewAny(User $user): bool
    {
        return true;
    }

    /**
     * Determine whether the crop can view the model.
     */
    public function view(User $user, Crop $model): bool
    {
        return true;
    }

    /**
     * Determine whether the crop can create models.
     */
    public function create(User $user): bool
    {
        return true;
    }

    /**
     * Determine whether the crop can update the model.
     */
    public function update(User $user, Crop $model): bool
    {
        return true;
    }

    /**
     * Determine whether the crop can delete the model.
     */
    public function delete(User $user, Crop $model): bool
    {
        return true;
    }

    /**
     * Determine whether the user can delete multiple instances of the model.
     */
    public function deleteAny(User $user): bool
    {
        return true;
    }

    /**
     * Determine whether the crop can restore the model.
     */
    public function restore(User $user, Crop $model): bool
    {
        return false;
    }

    /**
     * Determine whether the crop can permanently delete the model.
     */
    public function forceDelete(User $user, Crop $model): bool
    {
        return false;
    }
}
